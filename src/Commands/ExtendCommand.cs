using MapChanger.Models;
using MapChanger.Dependencies;
using MapChanger.Helpers;
using Microsoft.Extensions.Logging;
using SwiftlyS2.Shared;
using SwiftlyS2.Shared.Commands;
using SwiftlyS2.Shared.Players;

namespace MapChanger.Commands;

public class ExtendCommand
{
    private readonly ISwiftlyCore _core;
    private readonly PluginState _state;
    private readonly VoteManager _voteManager;
    private readonly ExtendManager _extendManager;
    private readonly MapChangerConfig _config;

    public ExtendCommand(ISwiftlyCore core, PluginState state, VoteManager voteManager, ExtendManager extendManager, MapChangerConfig config)
    {
        _core = core;
        _state = state;
        _voteManager = voteManager;
        _extendManager = extendManager;
        _config = config;
    }

    public void Execute(ICommandContext context)
    {
        if (!_config.ExtendMap.Enabled) return;
        if (!context.IsSentByPlayer) return;

        var player = context.Sender!;
        var localizer = _core.Translation.GetPlayerLocalizer(player);

        if (_state.WarmupRunning && !_config.ExtendMap.EnabledInWarmup)
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.general.validation.warmup"]);
            return;
        }

        // Refuse if a map change is already scheduled (RTV/EOF/admin) — extending
        // after a scheduled change has unclear semantics and can leak the timer.
        if (_state.MapChangeScheduled)
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.extend.map_change_scheduled"]);
            return;
        }

        // Refuse while the EOF map vote is currently happening — that vote has
        // its own "extend" option, which is the authoritative path.
        if (_state.EofVoteHappening)
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.extend.eof_vote_active"]);
            return;
        }

        // Cooldown between extend votes.
        if (_state.ExtendVoteCooldownEndTime.HasValue)
        {
            var remaining = _state.ExtendVoteCooldownEndTime.Value - DateTime.Now;
            if (remaining.TotalSeconds > 0)
            {
                player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.extend.cooldown_active", (int)Math.Ceiling(remaining.TotalSeconds)]);
                return;
            }
            _state.ExtendVoteCooldownEndTime = null;
        }

        if (_config.ExtendMap.MinPlayers > 0)
        {
            int playerCount = _core.PlayerManager.GetAllPlayers().Count(p => p.IsValid && !p.IsFakeClient);
            if (playerCount < _config.ExtendMap.MinPlayers)
            {
                player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.general.validation.min_players", _config.ExtendMap.MinPlayers]);
                return;
            }
        }

        if (_config.ExtendMap.MinRounds > 0)
        {
            int totalRoundsPlayed;
            try
            {
                totalRoundsPlayed = _core.Game.MatchData.TerroristScoreTotal + _core.Game.MatchData.CTScoreTotal;
            }
            catch (InvalidOperationException ex)
            {
                _core.Logger.LogWarning(ex, "GameRules not available in ExtendCommand - skipping MinRounds check");
                totalRoundsPlayed = _config.ExtendMap.MinRounds;
            }
            if (totalRoundsPlayed < _config.ExtendMap.MinRounds)
            {
                player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.general.validation.min_rounds", _config.ExtendMap.MinRounds - totalRoundsPlayed]);
                return;
            }
        }

        if (_state.ExtendsLeft <= 0)
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.extend.no_extends_left"]);
            return;
        }

        // Reject spectators (team 1) and unassigned (team 0) unless explicitly allowed.
        var teamNum = player.Controller?.TeamNum ?? 0;
        if (!_config.AllowSpectatorsToVote && teamNum <= 1)
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.general.validation.spectator"]);
            return;
        }

        if (_voteManager.AddVote(player.Slot))
        {
            var allPlayers = _core.PlayerManager.GetAllPlayers()
                .Where(p => p.IsValid && !p.IsFakeClient && (_config.AllowSpectatorsToVote || (p.Controller?.TeamNum ?? 0) > 1))
                .ToList();
            int totalPlayers = allPlayers.Count;
            int needed = _voteManager.GetRequiredVotes(totalPlayers, _config.ExtendMap.VotePercentage);

            _core.PlayerManager.SendChat(_core.Localizer["map_chooser.prefix"] + " " + _core.Localizer["map_chooser.extend.voted", player.Controller?.PlayerName ?? "Unknown", _voteManager.VoteCount, needed]);

            if (_voteManager.HasReached(totalPlayers, _config.ExtendMap.VotePercentage))
            {
                _voteManager.Clear();
                if (_config.ExtendMap.CooldownDuration > 0)
                    _state.ExtendVoteCooldownEndTime = DateTime.Now.AddSeconds(_config.ExtendMap.CooldownDuration);
                _extendManager.ExtendMap(_config.EndOfMap.ExtendTimeStep, _config.EndOfMap.ExtendRoundStep);
            }
        }
        else
        {
            player.SendChat(localizer["map_chooser.prefix"] + " " + localizer["map_chooser.extend.already_voted"]);
        }
    }
}
